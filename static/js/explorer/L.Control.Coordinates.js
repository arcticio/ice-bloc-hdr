...
L.Control.Coordinates = L.Control.extend({
	...
	onAdd: function(map) {
		...
		var link = L.DomUtil.create('a', className + "-image", this._labelcontainer);
		link.href = '#';
		...
		if (options.useLatLngOrder) {
			...
			mSpan = L.DomUtil.create("span", "", this._inputcontainer);
			this._inputM = this._createInput("inputM", this._inputcontainer);
		} else {
			xSpan = L.DomUtil.create("span", "", this._inputcontainer);
			this._inputX = this._createInput("inputX", this._inputcontainer);
			$(this._inputX).attr("size", 10);
			ySpan = L.DomUtil.create("span", "", this._inputcontainer);
			this._inputY = this._createInput("inputY", this._inputcontainer);
			$(this._inputY).attr("size", 10);
			mSpan = L.DomUtil.create("span", "", this._inputcontainer);
			this._inputM = this._createInput("inputM", this._inputcontainer);
			$(this._inputM).attr("size", 10);
		}
		xSpan.innerHTML = options.labelTemplateLng.replace(" {x}", "");
		ySpan.innerHTML = options.labelTemplateLat.replace(" {y}", "");
		mSpan.innerHTML = " Feature";

		L.DomEvent.on(this._inputX, 'keyup', this._handleKeypress, this);
		L.DomEvent.on(this._inputY, 'keyup', this._handleKeypress, this);
		L.DomEvent.on(this._inputM, 'keyup', this._handleKeypress, this);
	...
	_handleSubmit: function() {
		...
		var m = this._inputM.value;

		if (x !== undefined && y !== undefined && m !== undefined) {
			MAPS.setMarker(this._map, y, x, m);
	...
	expand: function(marker) {
		...
		if (!marker && MAPS.markers.length){
			if (this._map === MAPS.right.map && MAPS.markers[0]){
				marker = MAPS.markers[0];
			} 
			if (this._map === MAPS.left.map && MAPS.markers[1]){
				marker = MAPS.markers[1];
			} 
		}
		if (!marker) {
			var lat = MAPS.getCenter(this._map).lat.toFixed(5);
			var lng = MAPS.getCenter(this._map).lng.toFixed(5);
			marker = MAPS.setMarker(this._map, lat, lng, "name me");
		}

		this._inputY.value = marker._latlng.lat;
		this._inputX.value = marker._latlng.lng;
		this._inputM.value = marker.options.icon.options.html;
	...
	_switchUI: function(evt) {
		...
		if (this._showsCoordinates) {
			//show textfields
			// this.expand(); // no click function any longer
		} else {
			//show coordinates
			this.collapse();
		}
