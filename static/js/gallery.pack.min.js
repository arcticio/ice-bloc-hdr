var GALLERY=function(){var self,utcFormat=strftime.timezone(0);return{boot:function(){return self=this},utc:function(){var now=new Date;return new Date(Date.UTC(now.getUTCFullYear(),now.getUTCMonth(),now.getUTCDate(),now.getUTCHours(),now.getUTCMinutes(),now.getUTCSeconds(),now.getUTCMilliseconds()))},diffYM:function(dt,y,m){y=y||0;m=m||0;var date=new Date(dt);return(new Date(Date.UTC(date.getUTCFullYear()+y,date.getUTCMonth()+m,date.getUTCDate(),date.getUTCHours(),date.getUTCMinutes(),date.getUTCSeconds(),
date.getUTCMilliseconds()))).getTime()},diffMsecs:function(dt,off){var n=parseInt(off,10),o=off.slice(-1);return{"S":function(n){return dt+n*1E3},"M":function(n){return dt+n*1E3*60},"H":function(n){return dt+n*1E3*60*60},"d":function(n){return dt+n*1E3*60*60*24},"w":function(n){return dt+n*1E3*60*60*24*7},"m":function(n){return self.diffYM(dt,0,n)},"Y":function(n){return self.diffYM(dt,n)}}[o](n)},computeOffset:function(offset){var msecs=Date.now();offset.split("+").join("/+").split("-").join("/-").split("/").filter(function(t){return!!t}).forEach(function(token){msecs=
self.diffMsecs(msecs,token)});return new Date(msecs)},offFormat:function(offsetPattern){var tokens=offsetPattern.split(" "),offset=tokens[0],pattern=tokens.slice(1).join(" "),offTime=self.computeOffset(offset);return strftime.timezone(0)(pattern,offTime)},initQuicklinks:function(){$("#data .exp-quick-link").each(function(idx,ele){var link=$(ele).find("a"),anchor=link.attr("data-time-range"),img=$(ele).find("img"),span=$(ele).find(".exp-link-date"),range=dataTimeRanges[anchor],last=range[0][1],date=
H.iso2DayHour(last),pattern={"sic-amsr2":"//ice-pics.appspot.com/pics/?mime=png&width=128&url=http://noiv.pythonanywhere.com/data/amsr2/%Y/amsr2-%Y-%m-%d.png","sit-smos":"//ice-pics.appspot.com/pics/?mime=png&width=128&url=http://noiv.pythonanywhere.com/data/smos/%Y/smos2-%Y-%m-%d.png","sit-piomas":"//ice-pics.appspot.com/pics/?mime=png&width=128&url=http://noiv.pythonanywhere.com/data/piomas/%Y/piomas-%Y-%m.png","sit-cryosat":"//ice-pics.appspot.com/pics/?mime=png&width=128&url=http://noiv.pythonanywhere.com/data/cryosat/%Y/cryosat-%Y-%m.png",
"sst-avhrr":"//ice-pics.appspot.com/pics/?mime=png&width=128&url=http://noiv.pythonanywhere.com/data/sst/%Y/sst-%Y-%m-%d.png","gfs-forecast":"//ice-pics.appspot.com/pics/?mime=png&width=128&url=http://noiv.pythonanywhere.com/data/gfs/%Y/gfs-%Y-%m-%d-%H.png"}[anchor],src=strftime.timezone(0)(pattern,date);span.text(last);img.attr("src",src)})},initFigures:function(){var counter=0;$("#data .row, #weather .row").each(function(idx,ele){var groups=["test","ascat","cryosat","modis","zonalmean","dmitemps",
"observations","anomprob","icetemps","greenland"],caption,dataOff,row=$(ele),name=row.attr("data-name");if(groups.indexOf(name)!==-1){row.find("figure a").each(function(idx,ele){$(ele).attr("data-off")&&$(ele).attr("href",self.offFormat($(ele).attr("data-off")))});row.find("figure img").each(function(idx,ele){$(ele).attr("data-off")&&$(ele).attr("src",self.offFormat($(ele).attr("data-off")))});row.find("figure figcaption").each(function(idx,ele){if(dataOff=$(ele).attr("data-off")){caption=self.offFormat(dataOff);
$(ele).text(caption);$(ele).parent().find("img").attr("alt",caption)}});counter+=1}});return counter},init:function(){self.initQuicklinks();var groupsCOunter=self.initFigures();var stickyElements=initPhotoSwipeFromDOM("#sticky","figure");var dataElements=initPhotoSwipeFromDOM("#data .data-gallery","figure");var weatherElements=initPhotoSwipeFromDOM("#weather .data-gallery","figure");TIM.step("LOADED",["Gallery","groups",groupsCOunter,"elements",stickyElements,dataElements,weatherElements].join(", "))}}}().boot();
...
for(var i=0;i<type.length;i++){evName=type[i];if(evName)if(typeof listener==="object"&&listener.handleEvent){if(!unbind)listener["oldIE"+evName]=_handleEv;else if(!listener["oldIE"+evName])return false;target[methodName]("on"+evName,listener["oldIE"+evName])}else target[methodName]("on"+evName,listener)}};var self=this;var DOUBLE_TAP_RADIUS=25,NUM_HOLDERS=3;var _options={allowPanToNext:true,spacing:0.12,bgOpacity:1,mouseUsed:false,loop:true,pinchToClose:true,closeOnScroll:true,closeOnVerticalDrag:true,
...
speed){if(!s.backAnimStarted[axis]){if(_panOffset[axis]>_currPanBounds.min[axis])s.backAnimDestination[axis]=_currPanBounds.min[axis];else if(_panOffset[axis]<_currPanBounds.max[axis])s.backAnimDestination[axis]=_currPanBounds.max[axis];if(s.backAnimDestination[axis]!==undefined){s.slowDownRatio[axis]=0.7;s.slowDownRatioReverse[axis]=1-s.slowDownRatio[axis];if(s.speedDecelerationRatioAbs[axis]<0.05){s.lastFlickSpeed[axis]=0;s.backAnimStarted[axis]=true;_animateProp("bounceZoomPan"+axis,_panOffset[axis],
...
0:bounds.center.x;bounds.min.y=realPanElementH>_tempPanAreaSize.y?item.vGap.top:bounds.center.y},_calculateItemSize=function(item,viewportSize,zoomLevel){if(item.src&&!item.loadError){var isInitial=!zoomLevel;if(isInitial){if(!item.vGap)item.vGap={top:0,bottom:0};_shout("parseVerticalMargin",item)}_tempPanAreaSize.x=viewportSize.x;_tempPanAreaSize.y=viewportSize.y-item.vGap.top-item.vGap.bottom;if(isInitial){var hRatio=_tempPanAreaSize.x/item.w;var vRatio=_tempPanAreaSize.y/item.h;item.fitRatio=hRatio<
vRatio?hRatio:vRatio;var scaleMode=_options.scaleMode;if(scaleMode==="orig")zoomLevel=1;else if(scaleMode==="fit")zoomLevel=item.fitRatio;if(zoomLevel>1)zoomLevel=1;item.initialZoomLevel=zoomLevel;if(!item.bounds)item.bounds=_getZeroBounds()}if(!zoomLevel)return;_calculateSingleItemPanBounds(item,item.w*zoomLevel,item.h*zoomLevel);if(isInitial&&zoomLevel===item.initialZoomLevel)item.initialPosition=item.bounds.center;return item.bounds}else{item.w=item.h=0;item.initialZoomLevel=item.fitRatio=1;item.bounds=
_getZeroBounds();item.initialPosition=item.bounds.center;return item.bounds}},_appendImage=function(index,item,baseDiv,img,preventAnimation,keepPlaceholder){if(item.loadError)return;if(img){item.imageAppended=true;_setImageSize(item,img,item===self.currItem&&_renderMaxResolution);baseDiv.appendChild(img);if(keepPlaceholder)setTimeout(function(){if(item&&item.loaded&&item.placeholder){item.placeholder.style.display="none";item.placeholder=null}},500)}},_preloadImage=function(item){item.loading=true;
item.loaded=false;var img=item.img=framework.createEl("pswp__img","img");var onComplete=function(){item.loading=false;item.loaded=true;if(item.loadComplete)item.loadComplete(item);else item.img=null;img.onload=img.onerror=null;img=null};img.onload=onComplete;img.onerror=function(){item.loadError=true;onComplete()};img.src=item.src;return img},_checkForError=function(item,cleanUp){if(item.src&&item.loadError&&item.container){if(cleanUp)item.container.innerHTML="";item.container.innerHTML=_options.errorMsg.replace("%url%",
...
vars=hash.split("&");for(i=0;i<vars.length;i++){if(!vars[i])continue;var pair=vars[i].split("=");if(pair.length<2)continue;params[pair[0]]=pair[1]}if(_options.galleryPIDs){var searchfor=params.pid;params.pid=0;for(i=0;i<_items.length;i++)if(_items[i].pid===searchfor){params.pid=i;break}}else params.pid=parseInt(params.pid,10)-1;if(params.pid<0)params.pid=0;return params},_updateHash=function(){if(_hashAnimCheckTimeout)clearTimeout(_hashAnimCheckTimeout);if(_numAnimations||_isDragging){_hashAnimCheckTimeout=
...
"pushState"in history;if(_initialHash.indexOf("gid=")>-1){_initialHash=_initialHash.split("&gid=")[0];_initialHash=_initialHash.split("?gid=")[0]}_listen("afterChange",self.updateURL);_listen("unbindEvents",function(){framework.unbind(window,"hashchange",self.onHashChange)});var returnToOriginal=function(){_hashReseted=true;if(!_closedFromURL)if(_urlChangedOnce)history.back();else if(_initialHash)_windowLoc.hash=_initialHash;else if(_supportsPushState)history.pushState("",document.title,_windowLoc.pathname+
_windowLoc.search);else _windowLoc.hash="";_cleanHistoryTimeouts()};_listen("unbindEvents",function(){if(_closedByScroll)returnToOriginal()});_listen("destroy",function(){if(!_hashReseted)returnToOriginal()});_listen("firstUpdate",function(){_currentItemIndex=_parseItemIndexFromURL().pid});var index=_initialHash.indexOf("pid=");if(index>-1){_initialHash=_initialHash.substring(0,index);if(_initialHash.slice(-1)==="&")_initialHash=_initialHash.slice(0,-1)}setTimeout(function(){if(_isOpen)framework.bind(window,
...
""},getPageURLForShare:function(){return window.location.href},getTextForShare:function(){return pswp.currItem.title||""},indexIndicatorSep:" / ",fitControlsWidth:1200},_blockControlsTap,_blockControlsTapTimeout;var _onControlsTap=function(e){if(_blockControlsTap)return true;e=e||window.event;if(_options.timeToIdle&&_options.mouseUsed&&!_isIdle)_onIdleMouseMove();var target=e.target||e.srcElement,uiElement,clickedClass=target.getAttribute("class")||"",found;for(var i=0;i<_uiElements.length;i++){uiElement=
_uiElements[i];if(uiElement.onTap&&clickedClass.indexOf("pswp__"+uiElement.name)>-1){uiElement.onTap();found=true}}if(found){if(e.stopPropagation)e.stopPropagation();_blockControlsTap=true;var tapDelay=framework.features.isOldAndroid?600:30;_blockControlsTapTimeout=setTimeout(function(){_blockControlsTap=false},tapDelay)}},_fitControlsInViewport=function(){return!pswp.likelyTouchDevice||_options.mouseUsed||screen.width>_options.fitControlsWidth},_togglePswpClass=function(el,cName,add){framework[(add?
"add":"remove")+"Class"](el,"pswp__"+cName)},_countNumItems=function(){var hasOneSlide=_options.getNumItemsFn()===1;if(hasOneSlide!==_galleryHasOneSlide){_togglePswpClass(_controls,"ui--one-slide",hasOneSlide);_galleryHasOneSlide=hasOneSlide}},_toggleShareModalClass=function(){_togglePswpClass(_shareModal,"share-modal--hidden",_shareModalHidden)},_toggleShareModal=function(){_shareModalHidden=!_shareModalHidden;if(!_shareModalHidden){_toggleShareModalClass();setTimeout(function(){if(!_shareModalHidden)framework.addClass(_shareModal,
"pswp__share-modal--fade-in")},30)}else{framework.removeClass(_shareModal,"pswp__share-modal--fade-in");setTimeout(function(){if(_shareModalHidden)_toggleShareModalClass()},300)}if(!_shareModalHidden)_updateShareURLs();return false},_openWindowPopup=function(e){e=e||window.event;var target=e.target||e.srcElement;pswp.shout("shareLinkClick",e,target);if(!target.href)return false;if(target.hasAttribute("download"))return true;window.open(target.href,"pswp_share","scrollbars=yes,resizable=yes,toolbar=no,"+
"location=yes,width=550,height=420,top=100,left="+(window.screen?Math.round(screen.width/2-275):100));if(!_shareModalHidden)_toggleShareModal();return false},_updateShareURLs=function(){var shareButtonOut="",shareButtonData,shareURL,image_url,page_url,share_text;for(var i=0;i<_options.shareButtons.length;i++){shareButtonData=_options.shareButtons[i];image_url=_options.getImageURLForShare(shareButtonData);page_url=_options.getPageURLForShare(shareButtonData);share_text=_options.getTextForShare(shareButtonData);
shareURL=shareButtonData.url.replace("{{url}}",encodeURIComponent(page_url)).replace("{{image_url}}",encodeURIComponent(image_url)).replace("{{raw_image_url}}",image_url).replace("{{text}}",encodeURIComponent(share_text));shareButtonOut+='<a href="'+shareURL+'" target="_blank" '+'class="pswp__share--'+shareButtonData.id+'"'+(shareButtonData.download?"download":"")+">"+shareButtonData.label+"</a>";if(_options.parseShareButtonOut)shareButtonOut=_options.parseShareButtonOut(shareButtonData,shareButtonOut)}_shareModal.children[0].innerHTML=
shareButtonOut;_shareModal.children[0].onclick=_openWindowPopup},_hasCloseClass=function(target){for(var i=0;i<_options.closeElClasses.length;i++)if(framework.hasClass(target,"pswp__"+_options.closeElClasses[i]))return true},_idleInterval,_idleTimer,_idleIncrement=0,_onIdleMouseMove=function(){clearTimeout(_idleTimer);_idleIncrement=0;if(_isIdle)ui.setIdle(false)},_onMouseLeaveWindow=function(e){e=e?e:window.event;var from=e.relatedTarget||e.toElement;if(!from||from.nodeName==="HTML"){clearTimeout(_idleTimer);
...
if(_idleIncrement===2)ui.setIdle(true)},_options.timeToIdle/2)})},_setupHidingControlsDuringGestures=function(){_listen("onVerticalDrag",function(now){if(_controlsVisible&&now<0.95)ui.hideControls();else if(!_controlsVisible&&now>=0.95)ui.showControls()});var pinchControlsHidden;_listen("onPinchClose",function(now){if(_controlsVisible&&now<0.9){ui.hideControls();pinchControlsHidden=true}else if(pinchControlsHidden&&!_controlsVisible&&now>0.9)ui.showControls()});_listen("zoomGestureEnded",function(){pinchControlsHidden=
false;if(pinchControlsHidden&&!_controlsVisible)ui.showControls()})};var _uiElements=[{name:"caption",option:"captionEl",onInit:function(el){_captionContainer=el}},{name:"share-modal",option:"shareEl",onInit:function(el){_shareModal=el},onTap:function(){_toggleShareModal()}},{name:"button--share",option:"shareEl",onInit:function(el){_shareButton=el},onTap:function(){_toggleShareModal()}},{name:"button--zoom",option:"zoomEl",onTap:pswp.toggleDesktopZoom},{name:"counter",option:"counterEl",onInit:function(el){_indexIndicator=
el}},{name:"button--close",option:"closeEl",onTap:pswp.close},{name:"button--arrow--left",option:"arrowEl",onTap:pswp.prev},{name:"button--arrow--right",option:"arrowEl",onTap:pswp.next},{name:"button--fs",option:"fullscreenEl",onTap:function(){if(_fullscrenAPI.isFullscreen())_fullscrenAPI.exit();else _fullscrenAPI.enter()}},{name:"preloader",option:"preloaderEl",onInit:function(el){_loadingIndicator=el}}];var _setupUIElements=function(){var item,classAttr,uiElement;var loopThroughChildElements=function(sChildren){if(!sChildren)return;
var l=sChildren.length;for(var i=0;i<l;i++){item=sChildren[i];classAttr=item.className;for(var a=0;a<_uiElements.length;a++){uiElement=_uiElements[a];if(classAttr.indexOf("pswp__"+uiElement.name)>-1)if(_options[uiElement.option]){framework.removeClass(item,"pswp__element--disabled");if(uiElement.onInit)uiElement.onInit(item)}else framework.addClass(item,"pswp__element--disabled")}}};loopThroughChildElements(_controls.children);var topBar=framework.getChildByClass(_controls,"pswp__top-bar");if(topBar)loopThroughChildElements(topBar.children)};
ui.init=function(){framework.extend(pswp.options,_defaultUIOptions,true);_options=pswp.options;_controls=framework.getChildByClass(pswp.scrollWrap,"pswp__ui");_listen=pswp.listen;_setupHidingControlsDuringGestures();_listen("beforeChange",ui.update);_listen("doubleTap",function(point){var initialZoomLevel=pswp.currItem.initialZoomLevel;if(pswp.getZoomLevel()!==initialZoomLevel)pswp.zoomTo(initialZoomLevel,point,333);else pswp.zoomTo(_options.getDoubleTapZoom(false,pswp.currItem),point,333)});_listen("preventDragEvent",
function(e,isDown,preventObj){var t=e.target||e.srcElement;if(t&&t.getAttribute("class")&&e.type.indexOf("mouse")>-1&&(t.getAttribute("class").indexOf("__caption")>0||/(SMALL|STRONG|EM)/i.test(t.tagName)))preventObj.prevent=false});_listen("bindEvents",function(){framework.bind(_controls,"pswpTap click",_onControlsTap);framework.bind(pswp.scrollWrap,"pswpTap",ui.onGlobalTap);if(!pswp.likelyTouchDevice)framework.bind(pswp.scrollWrap,"mouseover",ui.onMouseOver)});_listen("unbindEvents",function(){if(!_shareModalHidden)_toggleShareModal();
if(_idleInterval)clearInterval(_idleInterval);framework.unbind(document,"mouseout",_onMouseLeaveWindow);framework.unbind(document,"mousemove",_onIdleMouseMove);framework.unbind(_controls,"pswpTap click",_onControlsTap);framework.unbind(pswp.scrollWrap,"pswpTap",ui.onGlobalTap);framework.unbind(pswp.scrollWrap,"mouseover",ui.onMouseOver);if(_fullscrenAPI){framework.unbind(document,_fullscrenAPI.eventK,ui.updateFullscreen);if(_fullscrenAPI.isFullscreen()){_options.hideAnimationDuration=0;_fullscrenAPI.exit()}_fullscrenAPI=
null}});_listen("destroy",function(){if(_options.captionEl){if(_fakeCaptionContainer)_controls.removeChild(_fakeCaptionContainer);framework.removeClass(_captionContainer,"pswp__caption--empty")}if(_shareModal)_shareModal.children[0].onclick=null;framework.removeClass(_controls,"pswp__ui--over-close");framework.addClass(_controls,"pswp__ui--hidden");ui.setIdle(false)});if(!_options.showAnimationDuration)framework.removeClass(_controls,"pswp__ui--hidden");_listen("initialZoomIn",function(){if(_options.showAnimationDuration)framework.removeClass(_controls,
"pswp__ui--hidden")});_listen("initialZoomOut",function(){framework.addClass(_controls,"pswp__ui--hidden")});_listen("parseVerticalMargin",_applyNavBarGaps);_setupUIElements();if(_options.shareEl&&_shareButton&&_shareModal)_shareModalHidden=true;_countNumItems();_setupIdle();_setupFullscreenAPI();_setupLoadingIndicator()};ui.setIdle=function(isIdle){_isIdle=isIdle;_togglePswpClass(_controls,"ui--idle",isIdle)};ui.update=function(){if(_controlsVisible&&pswp.currItem){ui.updateIndexIndicator();if(_options.captionEl){_options.addCaptionHTMLFn(pswp.currItem,
_captionContainer);_togglePswpClass(_captionContainer,"caption--empty",!pswp.currItem.title)}_overlayUIUpdated=true}else _overlayUIUpdated=false;if(!_shareModalHidden)_toggleShareModal();_countNumItems()};ui.updateFullscreen=function(e){if(e)setTimeout(function(){pswp.setScrollOffset(0,framework.getScrollY())},50);framework[(_fullscrenAPI.isFullscreen()?"add":"remove")+"Class"](pswp.template,"pswp--fs")};ui.updateIndexIndicator=function(){if(_options.counterEl)_indexIndicator.innerHTML=pswp.getCurrentIndex()+
1+_options.indexIndicatorSep+_options.getNumItemsFn()};ui.onGlobalTap=function(e){e=e||window.event;var target=e.target||e.srcElement;if(_blockControlsTap)return;if(e.detail&&e.detail.pointerType==="mouse"){if(_hasCloseClass(target)){pswp.close();return}if(framework.hasClass(target,"pswp__img"))if(pswp.getZoomLevel()===1&&pswp.getZoomLevel()<=pswp.currItem.fitRatio){if(_options.clickToCloseNonZoomable)pswp.close()}else pswp.toggleDesktopZoom(e.detail.releasePoint)}else{if(_options.tapToToggleControls)if(_controlsVisible)ui.hideControls();
else ui.showControls();if(_options.tapToClose&&(framework.hasClass(target,"pswp__img")||_hasCloseClass(target))){pswp.close();return}}};ui.onMouseOver=function(e){e=e||window.event;var target=e.target||e.srcElement;_togglePswpClass(_controls,"ui--over-close",_hasCloseClass(target))};ui.hideControls=function(){framework.addClass(_controls,"pswp__ui--hidden");_controlsVisible=false};ui.showControls=function(){_controlsVisible=true;if(!_overlayUIUpdated)ui.update();framework.removeClass(_controls,"pswp__ui--hidden")};
ui.supportsFullscreen=function(){var d=document;return!!(d.exitFullscreen||d.mozCancelFullScreen||d.webkitExitFullscreen||d.msExitFullscreen)};ui.getFullscreenAPI=function(){var dE=document.documentElement,api,tF="fullscreenchange";if(dE.requestFullscreen)api={enterK:"requestFullscreen",exitK:"exitFullscreen",elementK:"fullscreenElement",eventK:tF};else if(dE.mozRequestFullScreen)api={enterK:"mozRequestFullScreen",exitK:"mozCancelFullScreen",elementK:"mozFullScreenElement",eventK:"moz"+tF};else if(dE.webkitRequestFullscreen)api=
...
var initPhotoSwipeFromDOM=function(gallerySelector,imageSelector){var parseThumbnailElements=function(el){var thumbElements=document.querySelectorAll(gallerySelector)[0].querySelectorAll(imageSelector),numNodes=thumbElements.length,items=[],figureEl,linkEl,size,item;for(var i=0;i<numNodes;i++){figureEl=thumbElements[i];if(figureEl.nodeType!==1)continue;linkEl=figureEl.children[0];size=linkEl.getAttribute("data-size").split("x");item={src:linkEl.getAttribute("href"),w:parseInt(size[0],10),h:parseInt(size[1],
...
"FIGURE"});if(!clickedListItem)return;var clickedGallery=clickedListItem.parentNode,childNodes=clickedListItem.parentNode.childNodes,numChildNodes=childNodes.length,nodeIndex=0,index;var thumbElements=document.querySelectorAll(gallerySelector)[0].querySelectorAll(imageSelector);if(!thumbElements.length)console.log("No thumbs found",gallerySelector,imageSelector);for(var i=0;i<thumbElements.length;i++){if(thumbElements[i]===clickedListItem){index=nodeIndex;break}nodeIndex++}if(index>=0)openPhotoSwipe(index,
clickedGallery);return false};var photoswipeParseHash=function(){var hash=window.location.hash.substring(1),params={};if(hash.length<5)return params;var vars=hash.split("&");for(var i=0;i<vars.length;i++){if(!vars[i])continue;var pair=vars[i].split("=");if(pair.length<2)continue;params[pair[0]]=pair[1]}if(params.gid)params.gid=parseInt(params.gid,10);return params};var openPhotoSwipe=function(index,galleryElement,disableAnimation,fromURL){var pswpElement=document.querySelectorAll(".pswp")[0],gallery,
options,items;items=parseThumbnailElements(galleryElement);options={galleryUID:galleryElement.getAttribute("data-pswp-uid"),history:false,getThumbBoundsFn:function(index){var thumbnail=items[index].el.getElementsByTagName("img")[0],pageYScroll=window.pageYOffset||document.documentElement.scrollTop,rect=thumbnail.getBoundingClientRect();return{x:rect.left,y:rect.top+pageYScroll,w:rect.width}}};if(fromURL)if(options.galleryPIDs)for(var j=0;j<items.length;j++){if(items[j].pid==index){options.index=j;
break}}else options.index=parseInt(index,10)-1;else options.index=parseInt(index,10);if(isNaN(options.index))return;if(disableAnimation)options.showAnimationDuration=0;gallery=new PhotoSwipe(pswpElement,PhotoSwipeUI_Default,items,options);gallery.init()};var galleryElements=document.querySelectorAll(gallerySelector+" figure");for(var i=0,l=galleryElements.length;i<l;i++){galleryElements[i].setAttribute("data-pswp-uid",i+1);galleryElements[i].onclick=onThumbnailsClick}var hashData=photoswipeParseHash();
if(hashData.pid&&hashData.gid)openPhotoSwipe(hashData.pid,galleryElements[hashData.gid-1],true,true);return galleryElements.length};
